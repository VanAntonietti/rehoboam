...
pub trait IndexResolver<T: Clone> {
    fn resolve(index: usize, data: &[T]) -> &T;
}
...
impl IndexResolver<Color> for PingPongCycle {
    fn resolve(index: usize, data: &[Color]) -> &Color {
        let dbl_index = index % (2 * data.len());
        let final_index = if dbl_index < data.len() {
            dbl_index
        } else {
            2 * data.len() - 1 - dbl_index
        };
